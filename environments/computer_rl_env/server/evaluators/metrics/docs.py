...
try:
    ...
    deltaE_ciede2000 = None  # noqa: N816
    rgb2lab = None

logger = logging.getLogger("computer_rl_env.server.evaluators.metrics.docs")
...
def read_x11_image(filepath):
    ...
    """
    if Image is None:
        raise ImportError("PIL is required for read_x11_image")
    ...
    with open(filepath, "rb") as f:
        ...
        _ = struct.unpack(">I", header_data[4:8])[0]  # version (unused)
        _ = struct.unpack(">I", header_data[8:12])[0]  # pixmap_format (unused)
...
def contains_page_break(docx_file, rules):
    if not docx_file or Document is None:
        return 0
...
def compare_docx_files(file1, file2, **options):
    ...
    if file1.endswith(".docx") and file2.endswith(".docx"):
        if Document is None:
            logger.error("python-docx is required for .docx file comparison")
            return 0
        try:
            doc1 = Document(file1)
            doc2 = Document(file2)
        ...
        if load is None:
            logger.error("odfpy is required for .odt file comparison")
            return 0
        try:
            doc1 = load(file1)
            doc2 = load(file2)
...
def compare_init_lines(file1, file2):
    if not file1 or not file2 or Document is None:
        return 0
...
def compare_docx_tables(docx_file1, docx_file2):
    if not docx_file1 or not docx_file2 or Document is None:
        return 0
...
def compare_docx_images(docx_file1, docx_file2):
    if not docx_file1 or not docx_file2 or Document is None or Image is None:
        return 0
...
def compare_image_text(image_path, rule):
    if not image_path or easyocr is None:
        logger.error("easyocr is required for compare_image_text")
        return 0
    ...
    try:
                ...
                try:
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            subprocess.run(
                                ["xwdtopnm", image_path], stdout=subprocess.PIPE, check=True
                            )
                            with open(temp_image_path, "wb") as f:
                                ...
                                _ = subprocess.run(
                                    ["pnmtopng"], input=result.stdout, stdout=f, check=True
                                )
                            actual_image_path = temp_image_path
        ...
        try:
            ...
            # Extract text from OCR results (bbox, text, confidence)
            texts = [
                str(item[1]) if isinstance(item, (list, tuple)) and len(item) > 1 else str(item)
                for item in result
            ]
            extracted_text = " ".join(texts)

            # Log OCR results
            logger.info(f"OCR extracted texts: {texts}")
...
def compare_font_names(docx_file, rules: Dict[str, Any]):
    if not docx_file or Document is None:
        return 0
...
def is_first_line_centered(docx_file):
    if not docx_file or Document is None or WD_PARAGRAPH_ALIGNMENT is None:
        return 0
...
def check_tabstops(docx_file1, docx_file2, **kwargs) -> float:
    if not docx_file1 or not docx_file2 or Document is None or WD_TAB_ALIGNMENT is None:
        return 0.0
    ...
    # Check if all required section properties are available
...
def evaluate_colored_words_in_tables(file_path1, file_path2, **kwargs):
    if not file_path1 or not file_path2 or Document is None or RGBColor is None:
        return 0
...
def check_highlighted_words(file_path1, file_path2):
    if not file_path1 or not file_path2 or load is None or Span is None:
        return 0
...
def evaluate_strike_through_last_paragraph(file_path1, file_path2):
    if not file_path1 or not file_path2 or Document is None:
        return 0
...
def evaluate_conversion(file_path):
    if not file_path or Document is None:
        return 0
...
def check_italic_font_size_14(path1, path2):
    if not path1 or not path2 or Document is None:
        return 0
...
def evaluate_alignment(docx_path):
    if not docx_path or Document is None:
        return 0
...
def get_unique_train_ids(initial_file):  # fixed standard
    if not initial_file or Document is None:
        return set(), 0
...
def check_no_duplicates(initial_file, processed_file):
    if not initial_file or not processed_file or Document is None:
        return 0
...
def compare_references(file1, file2, **options):
    ...
    ref1_idx = (
        doc1_paragraphs.index(reference_indicator) if reference_indicator in doc1_paragraphs else -1
    )
    ref2_idx = (
        doc2_paragraphs.index(reference_indicator) if reference_indicator in doc2_paragraphs else -1
    )
...
def compare_unique_train_records(processed_file, expected_files, **kwargs):
    ...
    if Document is None:
        logger.error("python-docx is required for compare_unique_train_records")
        return 0
    ...
    def get_lines_and_ids_from_file(file_path: str):
        try:
            if Document is None:
    ...
    if processed_lines is None or processed_train_ids is None:
        return 0

    gold_lines, gold_train_ids = get_lines_and_ids_from_file(gold_file)
    if gold_lines is None or gold_train_ids is None:
        return 0
