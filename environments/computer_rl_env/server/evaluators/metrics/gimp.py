...
logger = logging.getLogger("computer_rl_env.server.evaluators.metrics.gimp")
...
def compare_image_list(
    ...
    pred_list: List[str] = (
        [pred_img_path_list] if not isinstance(pred_img_path_list, list) else pred_img_path_list
    )
    gold_list: List[str] = (
        [gold_img_path_list] if not isinstance(gold_img_path_list, list) else gold_img_path_list
    )

    for pred_img_path, gold_img_path in zip(pred_list, gold_list):
...
def get_gimp_export_path() -> Union[str, bool]:
    """Get the export path from GIMP configuration file."""
...
def check_file_exists(directory: str, filename: str) -> int:
...
def increase_saturation(image1_path: str, image2_path: str) -> float:
    """Check if saturation increased from image1 to image2."""
    ...
    def calculate_saturation(image: Image.Image) -> float:
        ...
        mean_saturation: float = stat.mean[0]
...
def decrease_brightness(image1_path: str, image2_path: str) -> float:
    """Check if brightness decreased from image1 to image2."""

    def calculate_brightness_gray(image: Image.Image) -> float:
    ...
    brightness1 = calculate_brightness_gray(image1)
    brightness2 = calculate_brightness_gray(image2)

    return 1.0 if brightness1 > brightness2 else 0.0


def find_yellow_triangle(image: NDArray) -> Tuple[int, int]:
    """Find the center of a yellow triangle in an image."""
...
def compare_triangle_positions(image1: str, image2: str) -> float:
    """Compare triangle positions in two images."""
    ...
    cx1, cy1 = find_yellow_triangle(img1)
    cx2, cy2 = find_yellow_triangle(img2)

    # calculate the distance between the center of the triangle and the center of the image
    center_distance1 = np.sqrt((cx1 - img1.shape[1] // 2) ** 2 + (cy1 - img1.shape[0] // 2) ** 2)
    center_distance2 = np.sqrt((cx2 - img2.shape[1] // 2) ** 2 + (cy2 - img2.shape[0] // 2) ** 2)

    return 1.0 if center_distance1 > center_distance2 else 0.0
...
def normalize_brightness(image: Image.Image, target_brightness: float) -> Image.Image:
    ...
    def point_transform(x: int) -> int:
        return min(255, max(0, int(x * factor)))

    return image.point(point_transform)


def measure_saturation(hsv_image: Image.Image) -> float:
...
def structure_check_by_mse(img1: Image.Image, img2: Image.Image, threshold: float = 0.03) -> bool:
...
def structure_check_by_ssim(img1: Image.Image, img2: Image.Image, threshold: float = 0.9) -> bool:
    ...
    try:
        ...
        try:
            ...
            similarity_result = ssim(array1, array2, win_size=win_size, channel_axis=2)
        except TypeError:
            # Older versions use multichannel
            similarity_result = ssim(array1, array2, win_size=win_size, multichannel=True)
        # Extract float value (SSIM can return tuple with gradient in some modes)
        similarity = (
            float(similarity_result)
            if not isinstance(similarity_result, tuple)
            else float(similarity_result[0])
        )
...
def check_brightness_decrease_and_structure_sim(
    src_path: Optional[str], tgt_path: Optional[str], threshold: float = 0.03
) -> float:
    """
...
def check_saturation_increase_and_structure_sim(
    src_path: Optional[str], tgt_path: Optional[str]
) -> float:
    """
...
def check_file_exists_and_structure_sim(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
...
def check_triangle_position(tgt_path: Optional[str]) -> float:
    """
...
def check_structure_sim(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
...
def check_structure_sim_resized(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
...
def check_contrast_increase_and_structure_sim(
    src_path: Optional[str], tgt_path: Optional[str]
) -> float:
    """
...
def check_config_status(
    actual_config_path: Optional[str], rule: Dict[str, Union[str, List[str]]]
) -> float:
    """
...
def check_image_size(src_path: Optional[str], rule: Dict[str, Union[int, bool]]) -> float:
    """
    ...
    # Check the size
    if rule.get("height") is not None:
        height_same = actual_height == rule["height"]
    else:
        height_same = True
    if rule.get("width") is not None:
...
def safe_open_image_with_retry(
    file_path: Optional[str], max_retries: int = 3, retry_delay: float = 0.5
...
def check_palette_and_structure_sim(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
...
def check_textbox_on_leftside(src_path: Optional[str]) -> float:
    """
    ...
    for y in range(height):
        for x in range(width):
            ...
            pixel_val = gray_image.getpixel((x, y))
            if (
                isinstance(pixel_val, (int, float)) and pixel_val < 128
            ):  # Arbitrary threshold for "dark"
...
def check_image_mirror(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
    ...
    # Check if the image is mirrored
    transposed_image = source_image.transpose(Image.Transpose.FLIP_LEFT_RIGHT)
...
def check_green_background(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
...
def check_sharper(src_path: Optional[str], tgt_path: Optional[str]) -> float:
    """
...
def check_image_file_size(src_path: Optional[str], rule: Dict[str, int]) -> float:
    """
    Check if the size of the src image is within specified max_size
    """
    if src_path is None:
        return 0.0
...
def check_structure_sim_with_threshold(
    src_path: Optional[str], tgt_path: Optional[str], **options: Union[float, int, str, bool]
) -> float:
    """
    ...
    ssim_threshold = float(options.get("ssim_threshold", 0.85))

    # Use both print and logging to ensure output is visible
    print("[IMAGE_COMPARISON] Starting comparison")
    ...
    logger.info("check_structure_sim_with_threshold: Starting comparison")
    logger.info(f"  Source image: {src_path}")
    logger.info(f"  Target image: {tgt_path}")
    ...
    try:
        ...
        logger.info(f"  Source image info: size={img_src.size}, mode={img_src.mode}")
        logger.info(f"  Target image info: size={img_tgt.size}, mode={img_tgt.mode}")
        ...
        if img_src.size != img_tgt.size:
            ...
            print(
                "[IMAGE_COMPARISON]   ⚠ Resizing source image to match target size for comparison"
            )
            logger.info(f"  Image size mismatch: src={img_src.size} vs tgt={img_tgt.size}")
            logger.info("  Resizing source image to match target size for comparison")
            ...
            logger.info(f"  Source image resized to: {img_src.size}")

        # Convert to RGB if needed
        if img_src.mode != "RGB":
            img_src = img_src.convert("RGB")
            logger.debug("  Converted source image to RGB")
        if img_tgt.mode != "RGB":
            img_tgt = img_tgt.convert("RGB")
            logger.debug("  Converted target image to RGB")
        ...
        if min_dim < 7:
            win_size = min_dim if min_dim % 2 == 1 else min_dim - 1
            if win_size < 1:
                logger.error("  Image too small for SSIM computation (min dimension < 1)")
                return 0.0
        else:
            win_size = 7

        print(f"[IMAGE_COMPARISON]   SSIM window size: {win_size}")
        logger.info(f"  SSIM window size: {win_size}")
        ...
        try:
            ...
            logger.info(f"  SSIM similarity score: {similarity:.6f}")
            logger.info(f"  SSIM threshold: {ssim_threshold:.6f}")
            logger.info(f"  Difference: {similarity - ssim_threshold:.6f}")
            ...
            if structure_same:
                ...
                print(
                    "[IMAGE_COMPARISON]   💡 Consider lowering threshold if images are visually identical"
                )
                logger.warning(
                    f"  ✗ Comparison FAILED: SSIM ({similarity:.6f}) < threshold ({ssim_threshold:.6f})"
                )
                logger.warning("  Consider lowering threshold if images are visually identical")
        ...
        logger.error(f"  check_structure_sim_with_threshold error: {type(e).__name__}: {str(e)}")
...
def check_image_sim(
    src_path: Optional[str], tgt_path: Optional[str], **kwargs: Union[float, int, str, bool]
) -> float:
    """Legacy wrapper for check_structure_sim"""
    return check_structure_sim(src_path, tgt_path)


def check_gimp_config(actual_path: Optional[str], rule: Dict[str, Union[str, List[str]]]) -> float:
    """Legacy wrapper for check_config_status"""
    return check_config_status(actual_path, rule)
